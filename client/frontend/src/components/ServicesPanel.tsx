...
export function ServicesPanel({ localServices, peers, refreshLocalServices }: Props) {
  ...
  // togglePause / removeExposed / retargetService used to live here
  // when 'Mening servislarim' was rendered inside this panel. They
  // moved to Portal.tsx (then down into the unified PeerServicesGrid)
  // when the own-services list was hoisted to the centre column. The
  // form here only needs ExposeService directly via submitExpose.
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="p-4 border-b border-white/5">
        ...
        {/* The own-services list (with pencil/pause/remove controls)
            and the down-rows diagnostic strip both moved to the
            unified services grid in the centre column. Keeping them
            here too would render the same data twice. ServicesPanel
            now scopes to: form (top) + Lokalda topilgan + LAN scan
            + Murakkab variant (below). */}
      </div>
